...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        ...
        const connectionProfilePath: string = path.join(homedir, '.fabric-vscode', 'v2', 'environments', 'Beelock-Cheyn', 'gateways', 'Org1 Gateway.json');
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway, transient?: fabricNetwork.TransientMap): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('Beelock-Cheyn', contractName);
        } else {
            contract = await network.getContract('Beelock-Cheyn');
        }
